...
import { LocationsRetrievedAction } from "../location/actions";
...
const adminReducer = (state = immutableState, action: any): any => {
  ...
  switch (action.type) {
    ...
    case LOCATIONS_RETRIEVED:
      ...
      (result as any).data.forEach((item: { id: any }) => {
        const match = combinedLocations.find(
          (location: { id: any }) => location.id === item.id
        );
        if (match == null) {
          combinedLocations = combinedLocations.concat(item);
        } else {
          combinedLocations = combinedLocations.map((location: { id: any }) =>
            location.id === item.id ? item : location
          );
        }
      });
    ...
    case LOADED_USERS:
      ...
      (result as any).data.forEach((item: { id: any }) => {
        const match = combinedUsers.find(
          (user: { id: any }) => user.id === item.id
        );
        if (match == null) {
          combinedUsers = combinedUsers.concat(item);
        } else {
          combinedUsers = combinedUsers.map((user: { id: any }) =>
    ...
    case INSTANCES_RETRIEVED:
      ...
      (result as any).data.forEach((item: { id: any }) => {
        const match = combinedInstances.find(
          (instance: { id: any }) => instance.id === item.id
        );
        if (match == null) {
          combinedInstances = combinedInstances.concat(item);
        } else {
          combinedInstances = combinedInstances.map((instance: { id: any }) =>
            instance.id === item.id ? item : instance
          );
        }
      });
    ...
    case INSTANCE_REMOVED:
      ...
      updateMap = new Map(state.get("instances"));
      let instances: any = updateMap.get("instances");
      instances = instances.filter(
        (instance: { id: any }) => instance.id !== result.id
      );
