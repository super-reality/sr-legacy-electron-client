...
type xrAuthState = typeof initialState;

const xrAuthSlice = createSlice({
  name: "auth",
  initialState,
  reducers: {
    actionProcessing: (
    ...
    loginUserSuccess: (
      state: xrAuthState,
      action: PayloadAction<LoginResultAction>
    ): void => {
      if (action.payload.authUser) {
        console.log("*****************XREngine Logged in****************");
