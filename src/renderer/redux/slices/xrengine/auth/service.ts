...
export function loginUserByOAuth(service: string) {
  return (dispatch: Dispatch, getState: any): any => {
    ...
    const redirectUrl = `${apiServer}/oauth/${service}?feathers_token=${token}&redirect=${JSON.stringify(
      redirectObject
    )}`;
...
export function loginUserByJwt(
  accessToken: string,
...
export function createMagicLink(
  ...
  return (dispatch: Dispatch): any => {
    ...
    const enableEmailMagicLink =
      (authConfig && authConfig.enableEmailMagicLink) ?? true;
    const enableSmsMagicLink =
      (authConfig && authConfig.enableSmsMagicLink) ?? false;
    ...
    client
      ...
      .then((res: any) => {
        ...
        dispatchAlertSuccess(
          dispatch,
          "Login Magic Link was sent. Please check your Email or SMS."
        );
      })
      .catch((err: any) => {
        console.log(err);
...
export function addConnectionByOauth(
...
export const updateUserSettings = (id: any, data: any) => async (
  dispatch: any
) => {
  ...
  return async (dispatch: Dispatch, getState: any) => {
    ...
    const [modelURL, thumbnailURL] = await Promise.all([
      client.service("upload-presigned").get("", {
        query: { type: "avatar", fileName: model.name, fileSize: model.size }
      }),
      client.service("upload-presigned").get("", {
        query: {
          type: "user-thumbnail",
    ...
    Object.keys(modelURL.fields).forEach((key) =>
      modelData.append(key, modelURL.fields[key])
    );
    ...
    axios
      ...
      .then((res) => {
        ...
        Object.keys(thumbnailURL.fields).forEach((key) =>
          thumbnailData.append(key, thumbnailURL.fields[key])
        );
        ...
        axios
          ...
          .then((res) => {
            ...
            Promise.all([
              ...
              .then((_) => {
                ...
                client
                  ...
                  .then((_) => {
                    ...
                    (Network.instance
                      .transport as any).sendNetworkStatUpdateMessage({
              ...
              .catch((err) => {
                ...
                client.service("upload-presigned").remove("", {
                  query: {
                    keys: [modelURL.fields.Key, thumbnailURL.fields.Key]
                  }
                });
              });
          })
          .catch((err) => {
            ...
            client
              ...
              .remove("", { query: { keys: [modelURL.fields.Key] } });
          });
      })
      .catch((err) => {
...
export function updateUserAvatarId(
  userId: string,
  avatarId: string,
  avatarURL: string,
  thumbnailURL: string
...
const loadAvatarForUpdatedUser = async (user) => {
  ...
  return new Promise(async (resolve) => {
    ...
    if (avatars?.data && avatars.data.length === 2) {
      const avatarURL =
        avatars?.data[0].staticResourceType === "avatar"
          ? avatars?.data[0].url
          : avatars?.data[1].url;
      const thumbnailURL =
        avatars?.data[0].staticResourceType === "user-thumbnail"
          ? avatars?.data[0].url
          : avatars?.data[1].url;

      networkUser.avatarDetail = {
        avatarURL,
        thumbnailURL,
      ...
      for (const key of Object.keys(Network.instance.networkObjects)) {
...
client.service("user").on("patched", async (params) => {
  ...
  if (selfUser.id === user.id) {
    ...
    if (selfUser.channelInstanceId !== user.channelInstanceId)
      store.dispatch(clearChannelLayerUsers());
    ...
    if (
      ...
      user.channelInstanceId === selfUser.channelInstanceId
    )
    ...
    if (user.channelInstanceId !== selfUser.channelInstanceId)
      store.dispatch(removedChannelLayerUser(user));
  }
});
...
client.service("location-ban").on("created", async (params) => {
  ...
  const selfPartyUser =
    party && party.partyUsers
      ? party.partyUsers.find((partyUser) => partyUser.userId === selfUser.id)
      : {};
  const currentLocation = state
    .get("locations")
    .get("currentLocation")
    .get("location");
  const { locationBan } = params;
  if (
    selfUser.id === locationBan.userId &&
    currentLocation.id === locationBan.locationId
  ) {
