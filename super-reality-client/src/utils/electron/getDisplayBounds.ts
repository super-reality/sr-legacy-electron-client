...
export default function getDisplayBounds(
  ...
  newBounds.x = Math.min(...displays.map((display: Electron.Display) => display.bounds.x));
  newBounds.y = Math.min(...displays.map((display: Electron.Display) => display.bounds.y));
